...
const FacebookFeaturedPost = () => {
  ...
  useEffect(() => {
    let mounted = true;
    (async () => {
      const { data, error } = await supabase.functions.invoke("facebook-feed", { body: { limit: 10 } });
      if (!mounted) return;
      if (error) {
        console.error(error);
      ...
      <article className="relative overflow-hidden rounded-lg border bg-card shadow-sm">
        <div className="relative aspect-[16/9]">
          ...
          <div className="absolute bottom-4 left-4 right-4">
            <h2 className="text-2xl md:text-3xl font-extrabold">Latest News from Achayans Media</h2>
